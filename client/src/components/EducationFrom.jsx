...
const EducationFrom = ({ data, onChange }) => {
    ...
    return (
        <div className='space-y-6'>
            <div className='flex items-center justify-between'>
                <div>
                    <h3 className='flex items-center gap-2 text-lg font-semibold text-gray-900 dark:text-white'>Education</h3>
                    <p className='text-sm text-gray-500 dark:text-slate-400'>Add your education details</p>
                </div>
                <button onClick={addEducation} className='flex items-center gap-2 px-3 py-1 text-sm bg-purple-100 dark:bg-purple-900/40 text-purple-700 dark:text-purple-400 rounded hover:bg-purple-200 dark:hover:bg-purple-900/60 transition-colors'>
                    ...
                    {data.map((education, index) => (
                        <div key={index} className='p-4 border border-gray-200 dark:border-slate-800 rounded-lg space-y-3 transition-colors'>
                            <div className='flex justify-between items-start'>
                                <h4 className="dark:text-white">Education #{index + 1}</h4>
